...
<div align="center">
  ...
  <img src="https://img.shields.io/badge/Spring%20Boot-2.7+-red.svg" alt="Spring Boot Version">
...
FitBuddy follows a microservices architecture pattern with the following components:
...
└─────────────────┘    └──────────────────┘    └─────────────────┘
         ...
         ▼                       ▼                       ▼
┌─────────────────┐    ┌──────────────────┐    ┌─────────────────┐
│   Dashboard     │    │   PostgreSQL     │    │   RabbitMQ      │
│   Display       │    │   (Shared DB)    │    │   + Redis       │
└─────────────────┘    └──────────────────┘    └─────────────────┘
```
...
- **Spring Boot**: Java microservice for analytics and calculations
...
- Java 11+ (for local analytics service development)
...
1. **Navigate to analytics service directory:**
   ```bash
...
- **[Spring Boot Analytics Guide](docs/SPRING_BOOT_ANALYTICS_LEARNING_GUIDE.md)** - Microservice development with Spring Boot
...
### Analytics
- `POST /api/analytics/calories/calculate` - Calculate calories burned
- `GET /api/analytics/calories/daily-summary/{user_id}` - Get daily calorie summary
- `POST /api/analytics/plans/generate/{user_id}` - Generate workout plan
...
- Spring Boot team for the robust microservice framework
